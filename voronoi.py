...
class Voronoi:
    ...
    def process_circle_event(self):
        ...
        if e.valid:
            ...
            # original Voronoi edge creation
            s = Segment(e.p)
    ...
    def arc_insert(self, p):
        ...
        if self.arc.p.x == p.x:
            ...
            seg = Segment(start, i.p, i.pnext.p)
            i.s1 = i.pnext.s0 = seg
            self.output.append(seg)
    ...
    def get_output(self):
        ...
        for s in self.output:
            if s.end:
                res.append((s.start.x, s.start.y, s.end.x, s.end.y))
        return res
...
def draw_voronoi(ax, points, color):
    ...
    for x0, y0, x1, y1 in v.get_output():
